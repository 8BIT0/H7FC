...
static void afatfs_sdcardReadComplete(sdcardBlockOperation_e operation, uint32_t sectorIndex, uint8_t *buffer, uint32_t callbackData)
{
    ...
    {
        if (afatfs.cacheDescriptor[i].state != AFATFS_CACHE_STATE_EMPTY && afatfs.cacheDescriptor[i].sectorIndex == sectorIndex)
        {
            if (buffer == NULL)
            {
...
static void afatfs_sdcardWriteComplete(sdcardBlockOperation_e operation, uint32_t sectorIndex, uint8_t *buffer, uint32_t callbackData)
{
    ...
    for (int i = 0; i < AFATFS_NUM_CACHE_SECTORS; i++)
    {
         ...
         */
        if (afatfs.cacheDescriptor[i].sectorIndex == sectorIndex && afatfs.cacheDescriptor[i].state == AFATFS_CACHE_STATE_WRITING)
        {
            if (buffer == NULL)
            {
...
bool afatfs_flush()
{
    if (afatfs.cacheDirtyEntries > 0)
    {
        ...
        for (int i = 0; i < AFATFS_NUM_CACHE_SECTORS; i++)
        {
            if (afatfs.cacheDescriptor[i].state == AFATFS_CACHE_STATE_DIRTY && !afatfs.cacheDescriptor[i].locked && (earliestSectorIndex == -1 || afatfs.cacheDescriptor[i].writeTimestamp < earliestSectorTime))
            {
...
static bool afatfs_parseMBR(const uint8_t *sector)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        if (partition[i].lbaBegin > 0 && (partition[i].type == MBR_PARTITION_TYPE_FAT32 || partition[i].type == MBR_PARTITION_TYPE_FAT32_LBA || partition[i].type == MBR_PARTITION_TYPE_FAT16 || partition[i].type == MBR_PARTITION_TYPE_FAT16_LBA))
        {
...
static bool afatfs_parseVolumeID(const uint8_t *sector)
{
    ...
    if (volume->bytesPerSector != AFATFS_SECTOR_SIZE || volume->numFATs != AFATFS_NUM_FATS || sector[510] != FAT_VOLUME_ID_SIGNATURE_1 || sector[511] != FAT_VOLUME_ID_SIGNATURE_2)
    {
        return false;
    }
...
static uint8_t *afatfs_fileLockCursorSectorForWrite(afatfsFilePtr_t file)
{
    ...
    else
    {
         ...
         */
        if (
            cursorOffsetInSector > 0 || offsetOfEndOfSector < file->logicalSize)
        {
            cacheFlags |= AFATFS_CACHE_READ;
        }
...
doMore:
    ...
    case AFATFS_TRUNCATE_FILE_UPDATE_DIRECTORY:
        ...
        if (status == AFATFS_OPERATION_SUCCESS)
        {
            if (opState->currentCluster == 0x0)
            { // current cluster 0 at this phase means it is an empty file
...
static void afatfs_fcloseContinue(afatfsFilePtr_t file)
{
     ...
     */
    if (file->type != AFATFS_FILE_TYPE_DIRECTORY && file->type != AFATFS_FILE_TYPE_FAT16_ROOT_DIRECTORY && (file->mode & (AFATFS_FILE_MODE_APPEND | AFATFS_FILE_MODE_WRITE)) != 0)
    {
