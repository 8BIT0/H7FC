...
static void sdcard_reset(void)
{
    if (SD_Init() != 0)
    {
        ...
        {
            sdcard.operationStartTime = Get_CurrentRunningMs();
...
doMore:
    ...
    case SDCARD_STATE_WAITING_FOR_WRITE:
        ...
        else if (Get_CurrentRunningMs() > sdcard.operationStartTime + SDCARD_TIMEOUT_WRITE_MSEC)
        {
            /*
             ...
             * them to reuse their buffer millis Seconds faster than they otherwise would.
             */
            sdcard_reset();
            goto doMore;
        }
    ...
    case SDCARD_STATE_READING:
        ...
        case SDCARD_RECEIVE_BLOCK_IN_PROGRESS:
            if (Get_CurrentRunningMs() <= sdcard.operationStartTime + SDCARD_TIMEOUT_READ_MSEC)
            {
    ...
    if (sdcard.state >= SDCARD_STATE_RESET && sdcard.state < SDCARD_STATE_READY && Get_CurrentRunningMs() - sdcard.operationStartTime > SDCARD_TIMEOUT_INIT_MILLIS)
    {
        sdcard_reset();
    }
